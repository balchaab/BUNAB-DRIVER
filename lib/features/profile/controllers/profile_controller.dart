...
class ProfileController extends GetxController implements GetxService{
  ...
  /// Synced from [profileInfo] on each [getProfileInfo] call (same field as backend `wallet_balance`).
  double get driverWalletBalance => profileInfo?.wallet?.walletBalance ?? 0;

  /// Drivers must not go online while wallet balance is below zero (matches backend rule).
  bool get canGoOnlineWithCurrentWallet => driverWalletBalance >= 0;
  ...
  Future<Response> getProfileInfo({bool silent = false}) async {
    ...
    if(response!.statusCode == 200){
      ...
      await _applyNegativeWalletOfflineRule();
      if (isOnline == "1") {
  ...
  /// If wallet is negative but status is still online, toggle off via API so UI matches backend rules.
  Future<void> _applyNegativeWalletOfflineRule() async {
    if (driverWalletBalance >= 0) return;
    if (isOnline != "1") return;
    final response = await profileServiceInterface.profileOnlineOffline();
    if (response != null && response.statusCode == 200) {
      isOnline = "0";
      profileInfo?.details?.isOnline = "0";
      stopLocationRecord();
      showCustomSnackBar('wallet_negative_forced_offline'.tr, isError: true);
    }
  }
