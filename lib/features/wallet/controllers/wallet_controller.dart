...
class WalletController extends GetxController implements GetxService {
  ...
  Future<void> getPaymentGetWayList() async {
  ...
  /// Default selection for top-up: Chapa when listed, otherwise first gateway.
  void selectChapaOrFirstGateway() {
    ...
    final chapaIdx = paymentGateways!.indexWhere((g) {
      final title = (g.gatewayTitle ?? '').toLowerCase();
      final key = (g.gateway ?? '').toLowerCase();
      return title.contains('chapa') || key.contains('chapa');
    });
    final idx = chapaIdx >= 0 ? chapaIdx : 0;
    setDigitalPaymentType(idx, paymentGateways![idx].gateway ?? '');
  }
